    ...
    \begin{enumerate}[(i)]
      \item  Seja \(C\subseteq T\) uma cadeia e seja
       ...
       \item Suponha que \(y\not\in b\). Vamos mostrar que \(y\) é comparável
        ...
        \item
Vamos começar mostrando que \(|b \cap T_\alpha| = 1\) ou \(0\), \(\forall \alpha\). De fato, suponhamos por absurdo que \(|b \cap T_\alpha| \geq 2\) para algum \(\alpha\). Tome \(x,y \in b \cap T_\alpha, x \neq y\). Como \(b\) é ordenado, podemos assumir \(x < y\). Mas então \(\{t \in T: t < x\} \subsetneq \{t \in T: t < y\}\) e então  \(\alpha = h(x) < h(y) = \alpha\), impossível. Logo \(|b \cap T_\alpha| = 1\) ou \(0\), \(\forall \alpha\).
\par 
Seja \(\alpha > \ell(b)\) e suponhamos por absurdo que \(|b \cap T_\alpha| = 1\). Logo exsite \(u \in T_\alpha\) tal que \(u \in b\), pelo exercício \(3.2.ii\), segue que \(\{t \in T: t < u\} \subset b\), donde \(h(u) \leq \ell(b)\), mas \(u \in T_\alpha\) e então \(\alpha = h(u) \leq \ell(b)\), impossível. Assim, como \(|b \cap T_\alpha| = 1\) ou \(0\) e \(|b \cap T_\alpha| \neq 1\), devemos ter \(|b \cap T_\alpha| = 0\).
\par 
O caso \(\alpha<l(b)\) faremos por indução. Claramente quando \(\alpha = 0\) é verdade. Seja \(\alpha < \ell(b)\) fixado e \(|b \cap T_\beta| = 1\), para \(\beta < \alpha\). \\
\textbf{Caso \(\alpha\) é ordinal sucessor:}
Digamos \(\alpha = \gamma+1\) e portanto, \(|b \cap T_\gamma| = 1\). Tome \(y \in b \cap T_\gamma\), de modo que \(\{t \in T:t \leq y\} \subset b\). Mas \(\ell(b) > \alpha > \gamma\), donde existe \(x \in b, x > y\). Pelo exercício 3.1.v, existe um único \(z \in T\) tal que \(y < z \leq x\) e \(z\) é sucessor imediato de \(y\). Pela maximidade de \(b\), segue que \(z \in b\). Além disso, \(h(z) = h(y) + 1 = \gamma + 1 = \alpha\), donde \(z \in b \cap T_\alpha\). Segue que \(|b \cap T_\alpha| = 1\).\\
...
\begin{proof}
  ...
  \begin{enumerate}[(i)]
    ...
    ordenado de acordo a contenção de funções usual.
...
\begin{proof}[Solução]
 ...
 com \(\sup\{\alpha_\eta\}=\kappa\). Assim basta construir uma árvore formada
 ...
 conjunto: \[T\doteq\big\{f\colon\beta\rightarrow\kappa\,\colon
   \beta<\kappa\text{ e }\eta_f=f(0)<\lambda\text{ e }\dom(f)<\alpha_{\eta_f}\text{ e }f(\eta)=0 \text{ para }\eta\neq 0\big\}\cup\big\{\varnothing\big\}\]
 Observe que \(T\) é ordenado pela ordem usual de funções e que para
 \(\varnothing\neq f,g\in T, f\leq g\iff f(0)=g(0), \dom f\subseteq\dom g\).
 Vamos provar que \(T\) é uma árvore:  \(\{\varnothing\}\) é o menor elemento de
 \(T\). Seja \(A_x=\{y\in T\,\colon y<x\}\) e observe que \(A_x\) possui o mesmo
 ...
 generalidade temos \(\dom f\subseteq\dom g\), logo \(f\leq g\). \(b_\eta\) é
 maximal pois suponhamos \(b_\eta\subsetneq c\) com \(c\) cadeia, então existe
 ...
 b_\xi\). Como \(b\) é maximal e \(b\subset b_\xi\) então \(b=b_\xi\) pois
 \(b_\xi\) é ramo (maximal). 
 
 Obrserve que construímos \(T\) para que \(b_\eta=\{f\in T: f(0)=\eta\}\) tenha
